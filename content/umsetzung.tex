\chapter{Realisierung}
	...
	Es gibt verschiedene Möglichkeiten, wie ein Schock Detektor in den bestehenden Aufbau des Clock Set
	...
	Beschleunigungssignale und die für die Unterscheidung zwischen Zugdurchfahrt und SchockEvent 
	...
	Auf beiden Mikrokontrollern läuft ein Betriebssystem. Jenes auf dem Uhrwerk ist mit den bestehenden Tasks
	...
	In jedem Fall muss das SchockEvent Signal über den Clock Controller abgesetzt werden, 
	...
	ein SchockEvent Signal über den bestehenden Zweidrahtbus abzusetzen, müsste untersucht werden. 
	...
	Sollte sich der Schock Detektor bewähren, kann dieser in einem weiteren Schritt weiter in den 
...
\begin{landscape}
	\begin{longtable}{p{0.5cm} p{6cm} p{2.5cm} p{6cm} p{8cm}} \toprule
		...
		\caption{Ideen für die Realisierung des Schock Detektors} 
...
\restoregeometry
	...
	Die Realisierung des Schock Detektors erfolgt gemäss der Realisierungsidee Nr. 4 in der
	...
	\paragraph{Schema}
	\autoref{fig:schema} zeigt das Schema des Schock Detektor Moduls. Grundsätzlich wurde darauf geachtet, 
	...
	Für die Speisung des Schock Detektion Moduls sind zwei Möglichkeiten vorgesehen. Einerseits kann
	...
	wurde. Zusätzlich kann bei einem Schock Event ein Warnsignal an den Clcok Controller gesendet
	...
	ungefähr Fr. 7.65 pro Schock Detektor Modul.
	\vspace{2cm}
	\begin{figure}
		\centering
		%\vspace{-1cm}
	...
	\end{figure}
	\newpage
	\paragraph{Abschätzung Stromverbrauch}
		\begin{longtable}{p{3cm} p{3cm} p{3cm} p{1cm}}\toprule
			\textbf{Komponente}	& \textbf{Bemerkungen} & \textbf{Stromverbrauch} & \textbf{Stück}  \\	
			\midrule
			\endhead
			\multicolumn{2}{l}{\emph{Fortsetzung auf nächster Seite}}	\\ \bottomrule \endfoot \endlastfoot 
			...
			\bottomrule					 	
			\caption{Abschätzung Stromverbrauch} 
			\label{tab:stromverbrauch}
		\end{longtable}

	

	\paragraph{Dokumente Bestückung}		
	\begin{figure}[H]
